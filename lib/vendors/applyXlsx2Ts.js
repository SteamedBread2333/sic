...
function getColDataByHeaderIdx(data, idx, standByIdx, offset = 2) {
  ...
  for (let i = offset; i < data.length; i++) {
    if (data[i] && data[i].length) {
      const text = data[i][idx] ? data[i][idx] : standByIdx ? data[i][standByIdx] : '';
      // Fix: replace full-width brackets with half-width brackets
      text.replace(/｛(\d+)｝/g, '{$1}');
      result[data[i][0]] = text;
    }
  }
  return result;
}
...
export default `;
      ...
      const outputFilePath = path.join(targerDir, key, `${filePath}${suffix}`);
